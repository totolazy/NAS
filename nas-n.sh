...
#    sudo ./nas-n.sh cleanup        立即执行一次交换目录清理
#    sudo ./nas-n.sh status         查看运行状态
#    sudo ./nas-n.sh ports          打印端口映射表
#    sudo ./nas-n.sh macmini        打印 Mac mini 侧对接参数（只打印，不写脚本）
#    sudo ./nas-n.sh uninstall      卸载（默认保留交换目录数据）
#    ./nas-n.sh --emit <name>       打印内置生成物（调试用）
#═══════════════════════════════════════════════════════════════════════════════
...
#───────────────────────────────────────────────────────────────────────────────
# 4. 清理旧一代部署（已废弃的「推送方案」：/etc/nas-n、/opt/nas-n、/var/lib/nas-n 那套）
#───────────────────────────────────────────────────────────────────────────────
remove_legacy() {
  title "检查旧一代部署（已废弃的推送方案 nas-n）"
  local found=0 u c
...
# 由 nas-n.sh 生成：归档助手（以 $PULL_USER 用户运行）用的最小配置
EXCHANGE=$EXCHANGE
...
# 由 nas-n.sh 生成，挂到容器 /etc/cont-init.d/99-aria2-dir。
...
EOF
  ...
  sync_cert)
    cat <<'EOS'
#!/usr/bin/env bash
# 由 nas-n.sh 生成：把 Caddy 为 HY2_SNI 签发的证书同步给 hysteria2，变了就重启
set -uo pipefail
