...
**归档后会自动把 qBittorrent 里对应的种子从面板删掉**（只删种子，不删文件）。原因：文件被搬走后
qB 会把这些种子标成 `missingFiles`，而**仍在下载中的多文件种子可能把已归档的那部分重新下载**，
形成「传了又下」的循环。删除判定很保守，三个条件都满足才删：

1. 状态不在下载类（下载中 / 排队 / 校验中 / 暂停中一律不碰）
2. 进度必须是 **100%**
3. 它的**每一个文件**在磁盘上都不存在（逐文件核对，且会把容器内路径 `/Mac`、`/downloads`
   映射回宿主机路径再判断）
